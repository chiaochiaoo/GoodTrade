...
class quick_model(model):
	...
	def model_load_levels(self):
		...
		for symbol,val in self.model.items():

			#symbol= symbol[:-2]
			symbol = symbol[:-3]
			df.loc[df['symbol']==symbol,'high'] = df.loc[df['symbol']==symbol]['high']*val
			df.loc[df['symbol']==symbol,'low'] = df.loc[df['symbol']==symbol]['low']*val
			df.loc[df['symbol']==symbol,'close'] = df.loc[df['symbol']==symbol]['close']*val
			df.loc[df['symbol']==symbol,'open'] = df.loc[df['symbol']==symbol]['open']*val

		x= df.groupby(['day','ts']).sum()
		g = (x[['open'	,'high',	'low','close']].max(axis=1) - x[['open'	,'high',	'low','close']].min(axis=1)).groupby('day')

		s=(g.max()-g.min())[:-1].mean()
		...
		print("!!!!!!",f,sec,t)
		self.historical_plus = [f,sec,t]
		self.historical_minus = np.array([f,sec,t])*-1
