...
class OneToN(ManagementStrategy):

	def __init__(self,symbol,tradingplan,N):

		super().__init__("Management: 1-to-N risk-reward ",N,symbol,tradingplan)

		self.manaTrigger = OneToNTrigger("one to N manage",self)
	...
	def on_loading_up(self): #call this whenever the break at price changes. 
		...
		"""
		self.tradingplan.data[TRIGGER_PRICE_1] = round(self.price+coefficient*self.gap,2)  #75%
		self.tradingplan.data[TRIGGER_PRICE_7] = round(self.price+coefficient*self.gap*N/2,2)  #fifth
		self.tradingplan.data[TRIGGER_PRICE_9] = round(self.price+coefficient*self.gap*N,2)  #FINAL
...
class OneToTwoWideStop(ManagementStrategy):

	def __init__(self,symbol,tradingplan):

		super().__init__("Management: 1-to-2 risk-reward ",symbol,tradingplan)

		self.manaTrigger = TwoToOneWideStopTrigger("manage",self)
